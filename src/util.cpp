...
int nAnonymizeMasternodeXchangeAmount = 1000;
...
#else
    const char* pszModule = "MasternodeXchange";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MNXCV2
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MNXCV2
    // Mac: ~/Library/Application Support/MNXCV2
    // Unix: ~/.MNXCV2
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MNXCV2";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MasternodeXchange.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "MasternodeXchange.conf";
               ...
               fprintf(ConfFile, "port=32229\n");
               fprintf(ConfFile, "rpcport=32230\n");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "MasternodeXchanged.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong MasternodeXchange will not work properly.");
