...
void Shutdown()
{
    ...
    RenameThread("MasternodeXchange-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: MasternodeXchange.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: MasternodeXchanged.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 32229)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeMasternodeXchangeamount=<n> " + _("Keep N MasternodeXchange anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MasternodeXchange is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MasternodeXchange is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MasternodeXchange version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "MasternodeXchange server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MasternodeXchange") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MasternodeXchange to complete") << "\n";
    ...
    nAnonymizeMasternodeXchangeAmount = GetArg("-anonymizeMasternodeXchangeamount", 0);
    if(nAnonymizeMasternodeXchangeAmount > 999999) nAnonymizeMasternodeXchangeAmount = 999999;
    if(nAnonymizeMasternodeXchangeAmount < 2) nAnonymizeMasternodeXchangeAmount = 2;
    ...
    LogPrintf("Anonymize MasternodeXchange Amount %d\n", nAnonymizeMasternodeXchangeAmount);
