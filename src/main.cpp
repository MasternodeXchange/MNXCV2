...
const string strMessageMagic = "MasternodeXchange Signed Message:\n";
...
bool AcceptToMemoryPool(CTxMemPool& pool, CTransaction &tx, bool fLimitFree,
                        bool* pfMissingInputs, bool fRejectMasternodexchangeFee, bool ignoreFees)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
...

    {
        ...
        if (fRejectMasternodexchangeFee && nFees > MIN_RELAY_TX_FEE * 10000)
            return error("AcceptableInputs: : masternodexchange fees %s, %d > %d",
                         hash.ToString(),
...
bool AcceptableInputs(CTxMemPool& pool, const CTransaction &txo, bool fLimitFree,
                         bool* pfMissingInputs, bool fRejectMasternodexchangeFee, bool isDSTX)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
        *pfMissingInputs = false;
...

    {
        ...
        if (fRejectMasternodexchangeFee && nFees > txMinFee * 10000)
            return error("AcceptableInputs: : masternodexchange fees %s, %d > %d",
                         hash.ToString(),
                         nFees, MIN_RELAY_TX_FEE * 10000);
...
bool CMerkleTx::AcceptToMemoryPool(bool fLimitFree, bool fRejectMasternodexchangeFee, bool ignoreFees)
{
    return ::AcceptToMemoryPool(mempool, *this, fLimitFree, NULL, fRejectMasternodexchangeFee, ignoreFees);
}
...
{
    RenameThread("MasternodeXchange-loadblk");
