...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMasternodeXchangeAmount"))
        settings.setValue("nAnonymizeMasternodeXchangeAmount", 1000);
    nAnonymizeMasternodeXchangeAmount = settings.value("nAnonymizeMasternodeXchangeAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeMasternodeXchangeAmount"))
        SoftSetArg("-anonymizeMasternodeXchangeamount", settings.value("nAnonymizeMasternodeXchangeAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeMasternodeXchangeAmount:
            return QVariant(nAnonymizeMasternodeXchangeAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeMasternodeXchangeAmount:
            nAnonymizeMasternodeXchangeAmount = value.toInt();
            settings.setValue("nAnonymizeMasternodeXchangeAmount", nAnonymizeMasternodeXchangeAmount);
            emit AnonymizeMasternodeXchangeAmountChanged(nAnonymizeMasternodeXchangeAmount);
            break;
        default:
            break;
        }
