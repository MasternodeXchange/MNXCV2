...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CMasternodeXchangeAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(CMasternodeXchangeAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        // reject masternodexchange fee
        unsigned int masternodexchangefee = (transaction.getTransactionSize() == 0 ? nAddresses : transaction.getTransactionSize());
        masternodexchangefee = ((MIN_RELAY_TX_FEE * masternodexchangefee) * 10000);
        if (nFeeRequired > masternodexchangefee){
            LogPrintf("nFeeRequired: %d -- MasternodexchangeFee: %d\n", nFeeRequired, masternodexchangefee);
            return MasternodexchangeFee;
        }
    }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CMasternodeXchangeAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CTxDestination dest = CMasternodeXchangeAddress(strAddress).Get();
    ...
    const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
    QString strAddress = QString::fromStdString(CMasternodeXchangeAddress(address).ToString());
...
{
    // MasternodeXchange context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CMasternodeXchangeAddress(address).ToString())].push_back(out);
    }
}
