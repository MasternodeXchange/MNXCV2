...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("MasternodeXchange"))
        return false;
...
{
    // Convert MasternodeXchange:// to MasternodeXchange:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("MasternodeXchange://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "MasternodeXchange:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MasternodeXchange.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for MasternodeXchange.lnk
...
{
    return GetAutostartDir() / "MasternodeXchange.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a MasternodeXchange.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=MasternodeXchange\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the MasternodeXchange app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add MasternodeXchange app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MasternodeXchange-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  MasternodeXchange-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("MasternodeXchange-Qt"));
