...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMasternodeXchangeAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMasternodeXchangeAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMasternodeXchangeAmount = strAnonymizeMasternodeXchangeAmount.remove(strAnonymizeMasternodeXchangeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMasternodeXchangeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMasternodeXchangeAmount*COIN) nMaxToAnonymize = nAnonymizeMasternodeXchangeAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMasternodeXchangeAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMasternodeXchangeAmount));
        strAnonymizeMasternodeXchangeAmount = strAnonymizeMasternodeXchangeAmount.remove(strAnonymizeMasternodeXchangeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMasternodeXchangeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeMasternodeXchangeAmount == 0){
